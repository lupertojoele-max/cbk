...
const MONDOKART_URL = 'https://www.mondokart.com/it/allineatori-per-catene-mondokart/';
...
async function scrapeProducts() {
  console.log('🚀 Starting Mondokart Chain Aligners scraper...\n');
  ...
  try {
        ...
        productCards.forEach(card => {
          try {
            ...
            if (name && priceEl) {
              ...
              // - Se NON c'è .regular-price -> .price è il prezzo normale
    ...
    const processedProducts = allProducts.map((product, index) => {
      ...
      return {
        id: `mk-allineatore-${index + 1}`,
        name: cleanedName,
        slug: slug,
        brand: brand,
        price: product.price,  // Prezzo originale/normale
        discountedPrice: product.discountedPrice,  // Prezzo scontato (solo se c'è sconto)
    ...
    // Save data
    const outputPath = path.join(__dirname, 'chain-aligners-data.json');
...
scrapeProducts().then(products => {
  console.log(`\n🎉 Scraping completed! ${products.length} chain aligners found.`);
}).catch(error => {
