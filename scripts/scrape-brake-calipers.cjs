...
const MONDOKART_URL = 'https://www.mondokart.com/it/pinze-freno-mondokart/';
...
async function scrapeProducts() {
  console.log('🚀 Starting Puppeteer scraper for brake calipers (with infinite scroll)...\n');
  ...
  try {
    ...
    // Take screenshot
    await page.screenshot({ path: 'mondokart-brake-calipers-full.png', fullPage: false });
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`✓ TOTAL: Found ${products.length} brake caliper products`);
    ...
    fs.writeFileSync(
      path.join(__dirname, 'brake-calipers-data.json'),
      JSON.stringify(productsData, null, 2)
    );
    console.log('\n💾 Saved product data to brake-calipers-data.json\n');
