...
console.log('🔧 Merging reinforcements products...\n');

// Read reinforcements data
const reinforcementsData = JSON.parse(fs.readFileSync(path.join(__dirname, 'reinforcements-data.json'), 'utf8'));
...
console.log(`📊 Reinforcements products to add: ${reinforcementsData.length}`);

// Get existing slugs to avoid duplicates
const existingSlugs = new Set(productsData.products.map(p => p.slug));

// Filter out duplicates
const newProducts = reinforcementsData.filter(p => !existingSlugs.has(p.slug));
...
async function processProducts() {
  ...
  for (let i = 0; i < newProducts.length; i++) {
    ...
    const product = {
      ...
      description: `${item.name} - Rinforzo e accessorio per assale kart di alta qualità`,
      inStock: true,
      specifications: {}
    };
  ...
  console.log(`\n✅ Added ${productsToAdd.length} new reinforcement products`);
