...
const MONDOKART_URL = 'https://www.mondokart.com/it/varie-per-assali-mondokart/';
...
function extractBrand(name) {
  const brandPatterns = [
    ...
    { pattern: /vamec/i, brand: 'Vamec' },
    { pattern: /skf/i, brand: 'SKF' },
    { pattern: /snr/i, brand: 'SNR' },
    { pattern: /ina/i, brand: 'INA' },
    { pattern: /fag/i, brand: 'FAG' },
  ];
...
async function scrapeProducts() {
  console.log('🚀 Starting Mondokart Axle Accessories scraper...\n');
  ...
  try {
    ...
    // Save data
    const outputPath = path.join(__dirname, 'axle-accessories-data.json');
...
scrapeProducts().then(products => {
  console.log(`\n🎉 Scraping completed! ${products.length} axle accessories found.`);
}).catch(error => {
