...
const MONDOKART_URL = 'https://www.mondokart.com/it/minuteria-forcelle-molle-mondokart/';
...
async function scrapeProducts() {
  console.log('🚀 Starting Puppeteer scraper for hardware, forks and springs (with infinite scroll)...\n');
  ...
  try {
    ...
    let maxScrollAttempts = 40; // Increased for potentially more products
    ...
    // Take screenshot
    await page.screenshot({ path: 'mondokart-hardware-forks-springs-full.png', fullPage: false });
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`✓ TOTAL: Found ${products.length} hardware, forks and springs products`);
    ...
    const productsData = products.map((product, idx) => {
      ...
      return {
        id: `mk-minuteria-forcelle-${idx + 1}`,
        name: product.name,
        slug,
        price: product.price,
    ...
    fs.writeFileSync(
      path.join(__dirname, 'hardware-forks-springs-data.json'),
      JSON.stringify(productsData, null, 2)
    );
    console.log('\n💾 Saved product data to hardware-forks-springs-data.json\n');
