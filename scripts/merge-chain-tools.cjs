...
console.log('🔧 Merging chain tools products...\n');

// Read chain tools data
const toolsData = JSON.parse(fs.readFileSync(path.join(__dirname, 'chain-tools-data.json'), 'utf8'));
...
console.log(`📊 Chain tools to add: ${toolsData.length}`);

// Get existing slugs to avoid duplicates
const existingSlugs = new Set(productsData.products.map(p => p.slug));

// Filter out duplicates
const newProducts = toolsData.filter(p => !existingSlugs.has(p.slug));
...
async function processProducts() {
  ...
  for (let i = 0; i < newProducts.length; i++) {
    ...
    const product = {
      ...
      description: `${item.name} - Estrattore/smagliatore per catena kart`,
      inStock: true,
      specifications: {}
    };
  ...
  console.log(`\n✅ Added ${productsToAdd.length} new chain tool products`);
