...
const MONDOKART_URL = 'https://www.mondokart.com/it/tubi-freno-mondokart/';
...
async function scrapeProducts() {
  console.log('🚀 Starting Puppeteer scraper for brake hoses...\n');
  ...
  try {
    ...
    await page.screenshot({ path: 'mondokart-page.png', fullPage: false });
    console.log('📸 Screenshot saved as mondokart-page.png');

    // Save HTML for debugging
    const html = await page.content();
    fs.writeFileSync('mondokart-full.html', html);
    console.log('💾 Saved full HTML to mondokart-full.html');
    ...
    fs.writeFileSync(
      path.join(__dirname, 'brake-hoses-data.json'),
      JSON.stringify(productsData, null, 2)
    );
    console.log('💾 Saved product data to brake-hoses-data.json\n');
