...
const MONDOKART_URL = 'https://www.mondokart.com/it/kit-revisione-birelart-mondokart/';
...
(async () => {
  console.log('🚀 Starting BirelArt Revision Kits scraper...\n');
  ...
  const products = await page.evaluate(() => {
    ...
    productCards.forEach((card, index) => {
      try {
        ...
        let brand = 'Birel';  // Default to Birel since this is BirelArt revision kits page

        console.log(`Product ${index + 1}: ${name} - ${price}€`);

        results.push({
          id: `mk-kit-revisione-birelart-${index + 1}`,
          name,
          price,
          brand,
          imageUrl,
  ...
  console.log(`\n✅ Found ${products.length} BirelArt revision kit products\n`);
  ...
  // Save to JSON
  const outputPath = path.join(__dirname, 'birelart-revision-kits-data.json');
