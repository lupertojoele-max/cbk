...
const MONDOKART_URL = 'https://www.mondokart.com/it/accessori-cerchi-mondokart/';
...
(async () => {
  console.log('🚀 Starting Wheel Accessories scraper...\n');
  ...
  const products = await page.evaluate(() => {
    ...
    productCards.forEach((card, index) => {
      try {
        ...
        else if (name.toLowerCase().includes('top-kart') || name.toLowerCase().includes('top kart') || name.toLowerCase().includes('topkart')) brand = 'Top-Kart';
        ...
        else if (name.toLowerCase().includes('nrg')) brand = 'NRG';
        else brand = 'Mondokart Racing';

        console.log(`Product ${index + 1}: ${name} - ${price}€`);

        results.push({
          id: `mk-acc-cerchi-${index + 1}`,
          name,
          price,
          brand,
          imageUrl,
  ...
  console.log(`\n✅ Found ${products.length} wheel accessory products\n`);
  ...
  // Save to JSON
  const outputPath = path.join(__dirname, 'wheel-accessories-data.json');
