...
console.log('🔧 Merging chains products...\n');

// Read chains data
const chainsData = JSON.parse(fs.readFileSync(path.join(__dirname, 'chains-data.json'), 'utf8'));
...
console.log(`📊 Chains to add: ${chainsData.length}`);

// Get existing slugs to avoid duplicates
const existingSlugs = new Set(productsData.products.map(p => p.slug));

// Filter out duplicates
const newProducts = chainsData.filter(p => !existingSlugs.has(p.slug));
...
async function processProducts() {
  ...
  for (let i = 0; i < newProducts.length; i++) {
    ...
    const product = {
      ...
      description: `${item.name} - Catena per kart di alta qualità`,
      inStock: true,
      specifications: {}
    };
  ...
  console.log(`\n✅ Added ${productsToAdd.length} new chain products`);
