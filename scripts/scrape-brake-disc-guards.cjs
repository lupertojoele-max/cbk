...
const MONDOKART_URL = 'https://www.mondokart.com/it/protezioni-disco-freno-mondokart/';
...
(async () => {
  console.log('🚀 Starting Brake Disc Guards scraper...\n');
  ...
  const products = await page.evaluate(() => {
    ...
    productCards.forEach((card, index) => {
      try {
        ...
        results.push({
          id: `mk-protezione-disco-${index + 1}`,
          name,
          price,
          brand,
          imageUrl,
  ...
  console.log(`\n✅ Found ${products.length} brake disc guard products\n`);
  ...
  // Save to JSON
  const outputPath = path.join(__dirname, 'brake-disc-guards-data.json');
