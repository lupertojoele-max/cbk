...
console.log('🔧 Merging chains 428 products...\n');

// Read chains data
const chainsData = JSON.parse(fs.readFileSync(path.join(__dirname, 'chains-428-data.json'), 'utf8'));
...
console.log(`📊 Chains 428 to add: ${chainsData.length}`);
...
async function processProducts() {
  ...
  for (let i = 0; i < newProducts.length; i++) {
    ...
    const product = {
      ...
      description: `${item.name} - Catena passo 428 per kart KZ 125cc`,
      inStock: true,
      specifications: {}
    };
  ...
  console.log(`\n✅ Added ${productsToAdd.length} new chain 428 products`);
