...
const MONDOKART_URL = 'https://www.mondokart.com/en/chassis-crg-mondokart/';
...
async function scrapeProducts() {
  console.log('🚀 Starting scraper for CRG chassis...\n');
  ...
  try {
    ...
    await page.screenshot({ path: 'mondokart-crg-chassis.png', fullPage: false });
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`✓ TOTAL: Found ${products.length} CRG chassis products`);
    console.log('='.repeat(60));

    // Extract category/class from product name
    ...
    const productsData = products.map((product, idx) => {
      ...
      return {
        ...
        mondokartUrl: product.productUrl || 'https://www.mondokart.com/en/chassis-crg-mondokart/'
      };
    });

    fs.writeFileSync(
      path.join(__dirname, 'crg-chassis-data.json'),
      JSON.stringify(productsData, null, 2)
    );
    console.log('\n💾 Saved to crg-chassis-data.json\n');
