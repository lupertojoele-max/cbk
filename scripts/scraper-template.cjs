/**
 * TEMPLATE SCRAPER MONDOKART
 *
 * Per usarlo:
 * 1. Copia questo file con un nuovo nome (es: scrape-CATEGORIA.cjs)
 * 2. Modifica MONDOKART_URL con l'URL della categoria
 * 3. Modifica ID_PREFIX per l'id dei prodotti
 * 4. Modifica OUTPUT_FILE per il nome del file output
 * 5. Esegui: node scrape-CATEGORIA.cjs
 */
...
// ==================== CONFIGURAZIONE ====================
const MONDOKART_URL = 'https://www.mondokart.com/it/CATEGORIA-MONDOKART/';
const ID_PREFIX = 'mk-prodotto';  // Diventerà mk-prodotto-1, mk-prodotto-2, etc.
const OUTPUT_FILE = 'categoria-data.json';
// ========================================================
...
function extractBrand(name) {
  const brandPatterns = [
    ...
    { pattern: /iris/i, brand: 'IRIS' },
    { pattern: /cz/i, brand: 'CZ' },
  ];
...
async function scrapeProducts() {
  console.log('🚀 Starting Mondokart scraper...\n');
  ...
  try {
    ...
    const processedProducts = allProducts.map((product, index) => {
      ...
      return {
        ...
        slug: slug,
        brand: brand,
        price: product.price,  // Prezzo originale/normale
        discountedPrice: product.discountedPrice,  // Prezzo scontato (solo se c'è sconto reale)
...
scrapeProducts().then(products => {
  console.log(`\n🎉 Scraping completed! ${products.length} products found.`);
}).catch(error => {
