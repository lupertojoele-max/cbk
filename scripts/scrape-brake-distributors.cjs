...
const MONDOKART_URL = 'https://www.mondokart.com/it/ripartitori-di-frenata-mondokart/';
...
async function scrapeProducts() {
  console.log('🚀 Starting Puppeteer scraper for brake distributors (with infinite scroll)...\n');
  ...
  try {
    ...
    // Take screenshot
    await page.screenshot({ path: 'mondokart-brake-distributors-full.png', fullPage: false });
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`✓ TOTAL: Found ${products.length} brake distributor products`);
    ...
    fs.writeFileSync(
      path.join(__dirname, 'brake-distributors-data.json'),
      JSON.stringify(productsData, null, 2)
    );
    console.log('\n💾 Saved product data to brake-distributors-data.json\n');
