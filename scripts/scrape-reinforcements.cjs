...
const MONDOKART_URL = 'https://www.mondokart.com/it/rinforzi-boccole-per-assale-mondokart/';
...
function extractBrand(name) {
  ...
  return 'Mondokart Racing';
}

async function scrapeProducts() {
  console.log('🚀 Starting Mondokart Reinforcements scraper...\n');
  ...
  try {
    ...
    // Save data
    const outputPath = path.join(__dirname, 'reinforcements-data.json');
    ...
    processedProducts.forEach(p => {
      console.log(`   - ${p.name.substring(0, 60)}... (${p.brand}) €${p.price}`);
    });
...
scrapeProducts().then(products => {
  console.log(`\n🎉 Scraping completed! ${products.length} reinforcement products found.`);
}).catch(error => {
