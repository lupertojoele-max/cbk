...
const MONDOKART_URL = 'https://www.mondokart.com/it/kit-completi-impianto-frenante-mondokart/';
...
(async () => {
  console.log('🚀 Starting Complete Brake Kits scraper...\n');
  ...
  const products = await page.evaluate(() => {
    ...
    productCards.forEach((card, index) => {
      try {
        ...
        else if (name.toLowerCase().includes('righetti') || name.toLowerCase().includes('ridolfi')) brand = 'Righetti Ridolfi';
        else brand = 'Mondokart Racing';

        console.log(`Product ${index + 1}: ${name} - ${price}€`);

        results.push({
          id: `mk-kit-completo-${index + 1}`,
          name,
          price,
          brand,
          imageUrl,
  ...
  console.log(`\n✅ Found ${products.length} complete brake kit products\n`);
  ...
  // Save to JSON
  const outputPath = path.join(__dirname, 'complete-brake-kits-data.json');
