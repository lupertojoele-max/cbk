...
const MONDOKART_URL = 'https://www.mondokart.com/it/dischi-freno-generici-mondokart/';
...
async function scrapeProducts() {
  console.log('🚀 Starting Puppeteer scraper for brake discs (with infinite scroll)...\n');
  ...
  try {
    ...
    let maxScrollAttempts = 40; // Increased for more products
    ...
    // Take screenshot
    await page.screenshot({ path: 'mondokart-brake-discs-full.png', fullPage: false });
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`✓ TOTAL: Found ${products.length} brake disc products`);
    ...
    fs.writeFileSync(
      path.join(__dirname, 'brake-discs-data.json'),
      JSON.stringify(productsData, null, 2)
    );
    console.log('\n💾 Saved product data to brake-discs-data.json\n');
