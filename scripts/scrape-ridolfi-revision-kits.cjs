...
const MONDOKART_URL = 'https://www.mondokart.com/it/kit-revisione-righetti-ridolfi-mondokart/';
...
(async () => {
  console.log('🚀 Starting Righetti Ridolfi Revision Kits scraper...\n');
  ...
  const products = await page.evaluate(() => {
    ...
    productCards.forEach((card, index) => {
      try {
        ...
        // Extract brand - Righetti Ridolfi
        let brand = 'Righetti Ridolfi';

        console.log(`Product ${index + 1}: ${name} - ${price}€`);

        results.push({
          id: `mk-kit-revisione-ridolfi-${index + 1}`,
          name,
          price,
          brand,
          imageUrl,
  ...
  console.log(`\n✅ Found ${products.length} Righetti Ridolfi revision kit products\n`);
  ...
  // Save to JSON
  const outputPath = path.join(__dirname, 'ridolfi-revision-kits-data.json');
