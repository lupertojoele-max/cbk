...
const MONDOKART_URL = 'https://www.mondokart.com/it/kit-revisione-intrepid-mondokart/';
...
(async () => {
  console.log('🚀 Starting Intrepid Revision Kits scraper...\n');
  ...
  const products = await page.evaluate(() => {
    ...
    productCards.forEach((card, index) => {
      try {
        ...
        let brand = 'Intrepid';  // Default to Intrepid since this is Intrepid revision kits page

        console.log(`Product ${index + 1}: ${name} - ${price}€`);

        results.push({
          id: `mk-kit-revisione-intrepid-${index + 1}`,
          name,
          price,
          brand,
          imageUrl,
  ...
  console.log(`\n✅ Found ${products.length} Intrepid revision kit products\n`);
  ...
  // Save to JSON
  const outputPath = path.join(__dirname, 'intrepid-revision-kits-data.json');
