...
const MONDOKART_URL = 'https://www.mondokart.com/it/kit-revisione-crg-mondokart/';
...
(async () => {
  console.log('🚀 Starting CRG Revision Kits scraper...\n');
  ...
  const products = await page.evaluate(() => {
    ...
    productCards.forEach((card, index) => {
      try {
        ...
        let brand = 'CRG';  // Default to CRG since this is CRG revision kits page

        console.log(`Product ${index + 1}: ${name} - ${price}€`);

        results.push({
          id: `mk-kit-revisione-${index + 1}`,
          name,
          price,
          brand,
          imageUrl,
  ...
  console.log(`\n✅ Found ${products.length} CRG revision kit products\n`);
  ...
  // Save to JSON
  const outputPath = path.join(__dirname, 'crg-revision-kits-data.json');
