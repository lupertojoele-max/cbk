...
console.log('🔧 Merging crowns products...\n');

// Read crowns data
const crownsData = JSON.parse(fs.readFileSync(path.join(__dirname, 'crowns-data.json'), 'utf8'));
...
console.log(`📊 Crowns to add: ${crownsData.length}`);

// Get existing slugs to avoid duplicates
const existingSlugs = new Set(productsData.products.map(p => p.slug));

// Filter out duplicates
const newProducts = crownsData.filter(p => !existingSlugs.has(p.slug));
...
async function processProducts() {
  ...
  for (let i = 0; i < newProducts.length; i++) {
    ...
    // Create product object - category is 'corone-catene' for the subcategory page
    const product = {
      id: item.id,
      name: item.name,
      slug: item.slug,
      ...
      description: `${item.name} - Corona per kart di alta qualità`,
      inStock: true,
      specifications: {}
    };
  ...
  console.log(`\n✅ Added ${productsToAdd.length} new crown products`);
