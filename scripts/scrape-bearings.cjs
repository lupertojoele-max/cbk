...
const MONDOKART_URL = 'https://www.mondokart.com/it/cuscinetti-per-assale-mondokart/';
...
(async () => {
  console.log('🚀 Starting Bearings scraper...\n');
  ...
  const products = await page.evaluate(() => {
    ...
    productCards.forEach((card, index) => {
      try {
        ...
        let brand = '';
        if (name.toLowerCase().includes('skf')) brand = 'SKF';
        else if (name.toLowerCase().includes('snr')) brand = 'SNR';
        else if (name.toLowerCase().includes('ina')) brand = 'INA';
        else if (name.toLowerCase().includes('fag')) brand = 'FAG';
        else if (name.toLowerCase().includes('nsk')) brand = 'NSK';
        else if (name.toLowerCase().includes('timken')) brand = 'Timken';
        else if (name.toLowerCase().includes('koyo')) brand = 'Koyo';
        else brand = 'Mondokart Racing';

        console.log(`Product ${index + 1}: ${name} - ${price}€`);

        results.push({
          id: `mk-cuscinetto-${index + 1}`,
          name,
          price,
          brand,
          imageUrl,
  ...
  console.log(`\n✅ Found ${products.length} bearing products\n`);
  ...
  const outputPath = path.join(__dirname, 'bearings-data.json');
