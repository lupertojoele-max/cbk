...
console.log('🔧 Merging chain guards products...\n');

// Read chain guards data
const guardsData = JSON.parse(fs.readFileSync(path.join(__dirname, 'chain-guards-data.json'), 'utf8'));
...
console.log(`📊 Chain guards to add: ${guardsData.length}`);

// Get existing slugs to avoid duplicates
const existingSlugs = new Set(productsData.products.map(p => p.slug));

// Filter out duplicates
const newProducts = guardsData.filter(p => !existingSlugs.has(p.slug));
...
async function processProducts() {
  ...
  for (let i = 0; i < newProducts.length; i++) {
    ...
    const product = {
      ...
      description: `${item.name} - Paracatena per kart`,
      inStock: true,
      specifications: {}
    };
  ...
  console.log(`\n✅ Added ${productsToAdd.length} new chain guard products`);
