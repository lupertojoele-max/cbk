...
const MONDOKART_URL = 'https://www.mondokart.com/it/corone-kz-mini-60-kf-mondokart/';
...
function extractBrand(name) {
  const brandPatterns = [
    ...
    { pattern: /first/i, brand: 'First' },
    { pattern: /ital/i, brand: 'Ital' },
  ];
...
async function scrapeProducts() {
  console.log('🚀 Starting Mondokart Crowns scraper...\n');
  ...
  try {
    ...
    // Save data
    const outputPath = path.join(__dirname, 'crowns-data.json');
...
scrapeProducts().then(products => {
  console.log(`\n🎉 Scraping completed! ${products.length} crowns found.`);
}).catch(error => {
