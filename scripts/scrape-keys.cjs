...
const MONDOKART_URL = 'https://www.mondokart.com/it/chiavette-per-assale-mondokart/';
...
(async () => {
  console.log('🚀 Starting Keys scraper...\n');
  ...
  const products = await page.evaluate(() => {
    ...
    productCards.forEach((card, index) => {
      try {
        ...
        else if (name.toLowerCase().includes('wildkart')) brand = 'WildKart';
        else brand = 'Mondokart Racing';

        console.log(`Product ${index + 1}: ${name} - ${price}€`);

        results.push({
          id: `mk-chiavetta-${index + 1}`,
          name,
          price,
          brand,
          imageUrl,
  ...
  console.log(`\n✅ Found ${products.length} key products\n`);
  ...
  const outputPath = path.join(__dirname, 'keys-data.json');
