...
const MONDOKART_URL = 'https://www.mondokart.com/it/kit-revisione-tonykart-mondokart/';
...
(async () => {
  console.log('🚀 Starting TonyKart Revision Kits scraper...\n');
  ...
  const products = await page.evaluate(() => {
    ...
    productCards.forEach((card, index) => {
      try {
        ...
        let brand = 'TonyKart OTK';  // Default to TonyKart since this is TonyKart revision kits page

        console.log(`Product ${index + 1}: ${name} - ${price}€`);

        results.push({
          id: `mk-kit-revisione-tonykart-${index + 1}`,
          name,
          price,
          brand,
          imageUrl,
  ...
  console.log(`\n✅ Found ${products.length} TonyKart revision kit products\n`);
  ...
  // Save to JSON
  const outputPath = path.join(__dirname, 'tonykart-revision-kits-data.json');
